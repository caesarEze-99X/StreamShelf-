...
export const useContentCardHelper = (contentCardInfo: Partial<Content>): ContentCardHelperHook => {
  const { overview: description, title, poster_path: bannerImg, backdrop_path } = contentCardInfo;
  const { navigate } = useNavigation<NativeStackNavigationProp<RootStackParamList>>();

  const onContentCardPressed = (): void => {
    if (description && bannerImg)
      navigate(ROUTE_NAME.CommonInfo, { title: title!, description, bannerImg, backdrop_path });
  };
